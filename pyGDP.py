...
class pyGDPwebProcessing():
    ...
    def _executeRequest(self, processid, inputs, output, verbose):
        ...
        """
        wps = WebProcessingService(self.wpsUrl)
    ...
    def submitCustomBioclim(self, processid="org.n52.wps.server.r.gridded_bioclim", 
                            outputfname=None, verbose=False, **kwargs):
        '''Makes a call to the R script which generates custom Bioclim variables
        processid is the locator of our R script.
        ...
        '''
        self._validate_bioclim_inputs(outputfname, verbose=False, **kwargs)
        inputs = self._parse_bioclim_inputs(**kwargs)
        resultfile = self._executeRequest(processid, inputs, "name", verbose)
        shutil.move(resultfile, outputfname)
        
    def _validate_bioclim_inputs(self, outputfname, **kwargs):
        ...
        try:
            ...
            raise Exception("Invalid OPeNDAP_URI provided:  " + kwargs["OPeNDAP_URI"])
        
        #Check that the passed variable names exist
        datatypes = self.getDataType(kwargs["OPeNDAP_URI"], False)
        for var in ['prcp_var', 'tmax_var', 'tmin_var', 'tave_var']:
        ...
        timerange = self.getTimeRange(kwargs["OPeNDAP_URI"], 
    ...
    def _parse_bioclim_inputs(self, **kwargs):
